...
versions AS (
    ...
    FROM
        adtech.app_sdks AS ap
    INNER JOIN target_app AS ta ON
...
grid AS (
    ...
    FROM
        versions AS v
    CROSS JOIN sdks AS s
    LEFT JOIN adtech.app_sdks AS a
        ON
            v.version_code_id = a.version_code_id
            AND s.sdk_id = a.sdk_id
            AND a.store_app = (
                SELECT store_app
                FROM
                    target_app
...
myall AS (
    ...
    LEFT JOIN adtech.companies AS co
        ON
            sd.company_id = co.id
