CREATE TABLE public.tiers (
    ...
    -- 'B2B Analytics SDK', 'B2B Premium Portal'   
    name TEXT NOT NULL,
...
CREATE TABLE public.tier_prices (
    ...
    -- (Stripe price ID or 'local_trial' if provider_name is 'local')
    provider_price_id TEXT NOT NULL UNIQUE,
    billing_cycle TEXT NOT NULL CHECK (
        billing_cycle IN ('monthly', 'yearly', 'manual')
    ),
    is_current BOOLEAN DEFAULT TRUE NOT NULL,
    created_at TIMESTAMPTZ DEFAULT NOW()
);
-- Ensure only one price is current per product, cycle, and provider
CREATE UNIQUE INDEX idx_single_current_price_per_product ON public.tier_prices (
    tier_id, billing_cycle, provider_name
