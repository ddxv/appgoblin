...
export function encodeBase64(bytes: Uint8Array, padding: boolean = true): string {
	let result = '';
...
export function decodeBase64(encoded: string, padding: boolean = true): Uint8Array {
	...
	for (let i = 0; i < encoded.length; i += 4) {
		...
		for (let j = 0; j < 4; j++) {
			...
			if (!padding && (i + j >= encoded.length || encoded[i + j] === '=')) {
				continue;
			}
