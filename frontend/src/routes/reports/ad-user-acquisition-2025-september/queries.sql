--
-- AppGoblin User Acquisition Report - September 2025
-- https://appgoblin.info/reports/ad-user-acquisition-2025-september
-- SQL Query Reproducibility Documentation
--
-- This file contains all SQL queries used to generate the data for the September 2025
...
-- - Data is filtered to the period: September 1, 2025 - September 30, 2025 (inclusive)
...
WHERE
    called_at >= '2025-09-01'
    AND called_at < '2025-10-01';
...
top_adv_per_phash AS (
    ...
    WHERE
        pub_inner.run_at >= '2025-09-01'
        AND pub_inner.run_at < '2025-10-01'
...
creative_stats AS (
    ...
    WHERE
        pub.run_at >= '2025-09-01'
        AND pub.run_at < '2025-10-01'
...
ad_network_domain_ids AS (
    ...
    WHERE
        vcasr_1.run_at >= '2025-09-01'
        AND vcasr_1.run_at < '2025-10-01'
    UNION
    SELECT
        cr_1.advertiser_store_app_id,
...
WHERE
    vcasr.run_at >= '2025-09-01'
    AND vcasr.run_at < '2025-10-01'
