--
-- AppGoblin User Acquisition Report - January 2026
-- https://appgoblin.info/reports/ad-user-acquisition-2026-january
-- SQL Query Reproducibility Documentation
--
-- This file contains all SQL queries used to generate the data for the January 2026
...
--
-- IMPORTANT NOTES:
-- - These queries are intended to be run against the AppGoblin database as of January 2026
-- - Data is filtered to the period: January 1, 2026 - January 31, 2026 (inclusive)
...
WITH advertiser_z_scores AS (
    ...
    FROM (
        SELECT
            ...
            sazh.ad_supported,
            -- Composite score for ranking (statistical significance + momentum)
            (
    ...
    WHERE rn <= 5  -- Top 5 per store per week
),
...
top_adv_per_phash AS (
    ...
    WHERE
        pub_inner.run_at >= '2026-01-01'
        AND pub_inner.run_at < '2026-02-01'
...
creative_stats AS (
    ...
    WHERE
        pub.run_at >= '2026-01-01'
        AND pub.run_at < '2026-02-01'
...
ad_network_domain_ids AS (
    ...
    WHERE
        vcasr_1.run_at >= '2026-01-01'
        AND vcasr_1.run_at < '2026-02-01'
    UNION
    SELECT
        cr_1.advertiser_store_app_id,
...
WHERE
    vcasr.run_at >= '2026-01-01'
    AND vcasr.run_at < '2026-02-01'
