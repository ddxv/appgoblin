...
<div class="container mx-auto px-3 md:px-8 xl:px-16 py-6 md:py-8">
	...
	<div class={sectionContainerClass}>
		...
		<div class={sectionDescriptionClass}>
			<p class="mt-2">
				Brand new game <span class={publisherTextColor}>{data.apps[0].app_name}</span>
				hit the charts at the end of March 2026 with a big boost from it's ads. This breakout game is
				developed by <a href="/developers/Gravity+Game+Tech">Gravity Game Tech</a> and was seen advertising
				on Admob using Adjust by AppLovin.
			</p>
			<p>
				A runner up was <span class={publisherTextColor}>{data.apps[1].app_name}</span> which
				interestingly was using <a href="/companies/appier.com">Appier</a> a Taiwan based DSP and adtech
				platform. That was cool to see it show up in a top spot as it's a smaller mobile DSP.
			</p>
			<p>
				Another interesting advertising app in March was <span class={publisherTextColor}
					>{data.apps[17].app_name}</span
				> notable for it's larger size and use of Branch for an MMP. This tracks as Branch though a smaller
				MMP is very solid with it's web to mobile attribution with a special focus on ecommerce apps.
				Something that AppsFlyer struggles with still.
			</p>
		</div>
